...
public class SepharothAI : EnemyAI, IMutualBossBehaviour
{
    ...
    {
        InstantiatedRoom ir = enemy.owningSpawner.instantiatedRoom;
        Grid grid = ir.grid;

        int randomIndex = Random.Range(0, ir.room.spawnPositionArray.Length);

        Vector3Int spawnCell = new Vector3Int(ir.room.spawnPositionArray[randomIndex].x, ir.room.spawnPositionArray[randomIndex].y, 0);
