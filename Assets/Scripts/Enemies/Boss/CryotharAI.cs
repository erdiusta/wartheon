            ...
            while (fireTimer < fireProjectileDuration)
            {
                ...
                if (firingIntervalTimer < 0f)
                {
                    if (firingDurationTimer >= 0)
                    {
                        ...
                        FireWeapon(false, 0, 0, 0, 0, cryotharPhase: CryotharPhase.IceProjectile);
                    }
                    else
                    {
                        // Reset timers
                    ...
                    if (collider.GetType() == typeof(PolygonCollider2D))
                    {
                        ...
                        if (playerHealth = collider.GetComponent<Health>())
                        {
                            ...
                            else
                            {
                                ...
                                HealthAuthorityResolver.GetAuthority(playerHealth.gameObject).ApplyDamage(0, ctx);
                            }
                        }
                    }
                }
        ...
        else if (frostWrymPhase == CryotharPhase.Icicle)
        {
            ...
            while (fireTimer < fireProjectileDuration)
            {
                ...
                if (firingIntervalTimer < 0f)
                {
                    ...
                    {
                        firingDurationTimer -= Time.deltaTime;
                        FireWeapon(false, 0, 0, 0, 0, cryotharPhase: CryotharPhase.Icicle);
                    }
                    else
                    {
                        // Reset timers
