...
public class Enemy : MonoBehaviour, IEnemyCombatData
{
    ...
    public void EnemyInitialization(EnemyDetailsSO enemyDetails, int enemySpawnNumber, DungeonLevelSO dungeonLevel, bool isMultiplayer)
    {
        ...
        StartCoroutine(MaterializeEnemy(isMultiplayer));
    ...
    IEnumerator MaterializeEnemy(bool isMultiplayer)
    {
        ...
        EnemyEnable(false, isMultiplayer);
        ...
        EnemyEnable(true, isMultiplayer);

        // Materializing completed
        isMaterializing = false;
    }

    private void EnemyEnable(bool isEnabled, bool isMultiplayer)
    {
        ...
        if (!isMultiplayer)
        {
            ...
            if (enemyAI != null && !enemyDetails.isEnemyBoss) aiRigidbody2D.enabled = isEnabled;
        }
        else
        {
            enemyAINetwork.enabled = isEnabled;
            if (enemyAINetwork != null && !enemyDetails.isEnemyBoss) aiRigidbody2D.enabled = isEnabled;
        }
    }
    ...
    {
        currentMoveSpeed = enemyDetails.movementDetails.GetBaseMaxMoveSpeed() + additionalSpeedModifier - speedReducer;
        aiRigidbody2D.canMove = true;
        aiRigidbody2D.speed = currentMoveSpeed;
    }

    #region INTERFACES
    public bool Isboss => enemyDetails.isEnemyBoss;
    ...
    public float FireResistance => enemyDetails.fireResistance;
    public float WaterResistance => enemyDetails.waterResistance;
    public float AirResistance => enemyDetails.airResistance;
    public float EarthResistance => enemyDetails.earthResistance;
    public float LightResistance => enemyDetails.lightResistance;
    public float DarkResistance => enemyDetails.darkResistance;
