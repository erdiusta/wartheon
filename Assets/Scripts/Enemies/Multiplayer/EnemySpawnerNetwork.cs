...
public class EnemySpawnerNetwork : NetworkBehaviour
{
    ...
    private void StaticEventHandler_OnEnemyKilledServer(EnemyKilledArgs enemyKilledArgs)
    {
        if (enemyKilledArgs.enemy.enemyDetails.enemyCategory == EnemyCategory.MainSlime)
        {
            ...
            EnemyDetailsSO minionDetails = WartheonDatabase.Instance.GetEnemy(minionCategory);
    ...
    public IEnumerator SpawnEnemiesRoutine()
    {
        ...
        if (InputManager.TutorialEnabled)
        {
            ...
            {
                CreateEnemy(WartheonDatabase.Instance.GetEnemy(randomEnemySelector.GetRandomCategory()), grid.CellToWorld(cellPosition));
                yield break;
            }
            ...
            {
                CreateEnemy(WartheonDatabase.Instance.GetEnemy(randomEnemySelector.GetRandomCategory()), grid.CellToWorld(cellPosition - new Vector3Int(4, 4, 0)));
                yield break;
            }
        }
    ...
    public void CreateEnemy(EnemyDetailsSO enemyDetails, Vector3 position)
    {
        ...
        // Add room to enemy
        enemy.GetComponent<Enemy>().belongingRoomData = currentRoomNetData;
