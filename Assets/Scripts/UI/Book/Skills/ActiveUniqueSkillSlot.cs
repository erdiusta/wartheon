...
public class ActiveUniqueSkillSlot : MonoBehaviour, IDropHandler, ISelectHandler, IDeselectHandler
{
    ...
    IEnumerator WaitForPlayerInitialization()
    {
        while (player == null || !player.IsLocal)
        {
            ...
            if (player == null) Debug.Log("Player is not assigned in active skill slots.");
            else Debug.Log("Player is assigned successfully in active skill slots.");
    ...
    private void Update()
    {
        if (!isSelected || player == null || !player.IsLocal) return;
