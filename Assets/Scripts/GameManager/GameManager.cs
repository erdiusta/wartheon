...
#region Multiplayer Initialization
    ...
    {
        localPlayer.movementByForce.moveSpeed = localPlayer.movementByForce.movementDetails.GetBaseMaxMoveSpeed() + localPlayer.CurrentAgilityValue;

        if (!NetworkServer.active && !NetworkClient.active)
        {
            localPlayer.animator.speed = localPlayer.movementByForce.moveSpeed / Settings.baseSpeedForPlayerAnimations;
        }
    ...
    public void ServerTogglePause_SP()
    {
        ...
        ApplyPauseUI(!paused, true);
    }

    /// <summary>
    /// Pause game menu - also called from resume game button on pause menu
    /// </summary>
    public void ApplyPauseUI(bool paused, bool singlePlayer)
    {
        if (paused)
        {
