...
public class Projectile : MonoBehaviour, IFireable
{
            ...
            if (collision.collider.GetComponent<Enemy>() != null)
            {
                if (enemy.enemyDetails.hasShield)
                {
                    ...
                    if (deflectHappened)
                    {
                        ...
                        HealthAuthorityResolver.GetAuthority(enemy.gameObject).ApplyDamage(0, ctx);
                    }
                    else
                    {
                        // Deal Damage To Collision Object
    ...
    private void DealDamage(Collider2D collision, ref int inflictedDamage, bool isLaser = false)
    {
        ...
        if (health != null)
        {
            ...
            HealthAuthorityResolver.GetAuthority(health.gameObject).ApplyDamage(inflictedDamage, ctx, target);
        }
    }
    ...
    private void UpdateGrappleHook()
    {
        ...
        if (hit.collider != null)
        {
            if (hit.collider.CompareTag(Settings.enemyTag) && !grappleHasDealtDamage && currentRopeLength >= targetRopeLength - 0.01f)
            {
                ...
                HealthAuthorityResolver.GetAuthority(hit.transform.gameObject).ApplyDamage(inflictedDamage, ctx);
        ...
        for (int i = 0; i < hitCount; i++)
        {
            ...
            if (collider is PolygonCollider2D)
            {
                if (isEnemy)
                {
                    ...
                    if (collider.tag == Settings.playerTag)
                    {
                        ...
                        HealthAuthorityResolver.GetAuthority(player.gameObject).ApplyDamage(inflictedDamage, ctx);
                        CheckStunStatus(player);
            ...
            else
            {
                if (isEnemy)
                {
                    ...
                    HealthAuthorityResolver.GetAuthority(collider.gameObject).ApplyDamage(Random.Range(projectileDetails.burstDamageMin, projectileDetails.burstDamageMax), ctx);
                }
                else
                {
                    if (isFireBlast)
                    {
                        ...
                        HealthAuthorityResolver.GetAuthority(collider.gameObject).ApplyDamage(inflictedDamage, ctx);
                    }
                }
            }
        }
