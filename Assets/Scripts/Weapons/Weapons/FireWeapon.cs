...
public class FireWeapon : MonoBehaviour
{
    ...
    {      
        ...
        if (!NetworkServer.active && !NetworkClient.active)
        {
            ...
            spawnPositionArray = currentRoom.spawnPositionArray;
            lowerBounds = currentRoom.templateLowerBounds;
            upperBounds = currentRoom.templateUpperBounds;
            targetPlayer = GameManager.Instance.GetPlayer();
        }
        else
        {
            ...
            grid = instantiatedRoom.grid;
            spawnPositionArray = currentRoomNetData.spawnPositions;
            lowerBounds = currentRoomNetData.templateLowerBounds;
            upperBounds = currentRoomNetData.templateUpperBounds;
            targetPlayer = GameSessionManager.Instance.GetClosestPlayer(transform.position);
        }
        ...
        while (projectileCounter < projectilePerShot)
        {
                ...
                fireWrymPhase == PyrotharPhase.FirePillar || moldranPhase == MoldranPhase.Spike)
            {
                ...
                {
                    playerCellPosition = instantiatedRoom.grid.WorldToCell(GameManager.Instance.GetPlayer().transform.position);
                }
