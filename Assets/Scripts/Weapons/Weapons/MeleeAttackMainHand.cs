...
public class MeleeAttackMainHand : MonoBehaviour
{
        ...
        bool isDontBlink = false, bool isWhisperSlice = false)
    {
        ...
        foreach (var collider in hitSpan)
        {
                ...
                collider.TryGetComponent(out Health envHealth))
            {
                ...
                GameObject target = environment.TryGetComponent(out RoomProp prop) ? prop.gameObject : null;
                ...
                HealthAuthorityResolver.GetAuthority(envHealth.gameObject).ApplyDamage(100, ctx, target);
                continue;
            }
            ...
            IEnemyCombatData enemyCombatData = EnemyCombatDataResolver.Resolve(enemy.gameObject);
            ...
            if (attackHits)
            {
                ...
                if (shieldBash)
                {
                    ...
                    if (mainHandWeapon != null)
                    {
                        ...
                        if (enemy.health != null)
                        {
                            ...
                            HealthAuthorityResolver.GetAuthority(enemy.gameObject).ApplyDamage(inflictedDamage, ctxShieldBash);
                        }
                    }
                ...
                if (enemyHealth.suddenDeathHappened)
                {
                    ...
                    HealthAuthorityResolver.GetAuthority(enemyHealth.gameObject).ApplyDamage(enemyHealth.GetCurrentHealth() + 10, ctxSuddenDeath);
                ...
                HealthAuthorityResolver.GetAuthority(enemy.gameObject).ApplyDamage(inflictedDamage, ctx);
            ...
            else
            {
                ...
                HealthAuthorityResolver.GetAuthority(enemyHealth.gameObject).ApplyDamage(0, ctx);
    ...
    private void CheckShatterStatus(Enemy enemy, IEnemyCombatData enemyCombatData, ref int inflictedDamage)
    {
        ...
        if (isFrozen)
        {
            ...
            if (randomDice < 0.25f)
            {
                ...
                HealthAuthorityResolver.GetAuthority(enemyHealth.gameObject).ApplyDamage(inflictedDamage, ctx);
    ...
    private void DummyCheck(Collider2D collider, MeleeHand hand)
    {
        ...
        GameObject target = health.TryGetComponent(out RoomProp prop) ? prop.gameObject : null;

        DamageContext ctx = new DamageContext { source = DamageSourceType.Melee, dealerPosition = transform.position, receiverPosition = health.transform.position, hand = hand, bypassImmunity = bypass };
        HealthAuthorityResolver.GetAuthority(health.gameObject).ApplyDamage(damageDone, ctx, target);
    }
