...
public class AimWeapon : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            if(NetworkServer.active) mainHandShootPosition = enemy.enemyAINetwork?.weaponShootPosition;
            else mainHandShootPosition = enemy.enemyAI?.weaponShootPosition;
        }
    }
    ...
    /// Aim the weapon
    /// </summary>
    public void Aim(AimDirection aimDirection, AttackDirection attackDirection, float aimAngle, EnemyCategory enemyCategory)
    {
        if (tag == Settings.enemyTag)
        {
            mainHandWeaponRotationPointTransform.eulerAngles = new Vector3(0f, 0f, aimAngle);

            EnemyDetailsSO enemyDetails = NetworkServer.active ? WartheonDatabase.Instance.GetEnemy(enemyCategory) : enemy.enemyDetails;

            if (enemy.enemyDetails.enemyWeapon != null && enemyDetails.enemyWeapon.weaponClass == WeaponClass.Staff)
            {
                switch (aimDirection)
                {
            ...
            if (enemyDetails.enemyWeapon != null)
            {
                ...
                {
                    case AimDirection.Up:
                        mainHandShootPosition.localPosition = enemyDetails.enemyWeapon.weaponUpShootPosition;
                        break;
                    case AimDirection.Down:
                        mainHandShootPosition.localPosition = enemyDetails.enemyWeapon.weaponDownShootPosition;
                        break;
                    case AimDirection.Right:
                        mainHandShootPosition.localPosition = enemyDetails.enemyWeapon.weaponRightShootPosition;
                        break;
                    case AimDirection.Left:
                        mainHandShootPosition.localPosition = enemyDetails.enemyWeapon.weaponLeftShootPosition;
                        break;
                    default:
                        break;
                }
        ...
        else if (tag == Settings.playerTag)
        {
            ...
            Weapon playerMainHandWeapon = player.activeWeapon.GetCurrentMainHandWeapon();
    ...
    public void CrossbowAim(AimDirection aimDirection)
    {
        switch (aimDirection)
        {
            case AimDirection.Up:
    ...
    public void BowAim(AimDirection aimDirection)
    {
        switch (aimDirection)
        {
            case AimDirection.Up:
