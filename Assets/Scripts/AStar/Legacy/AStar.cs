...
{
    /// <summary>
    /// Builds a path for the room, from the startGridPosition to the endGridPosition, and adds movement steps to the returned Stack. Returns null 
    /// if no path is found
    /// </summary>
    ...
    private static int GetDistance(Node nodeA, Node nodeB)
    {
        ...
        if (distanceX > distanceY)
            return 14 * distanceY + 10 * (distanceX - distanceY);  // 10 used instead of 1, and 14 is a pythagoras approximation SQRT(10*10 + 10*10)
    ...
    /// </summary>
    private static Node GetValidNodeNeighbor(int neighborNodeXPosition, int neighborNodeYPosition, GridNodes gridNodes, HashSet<Node> closedNodeHashSet, 
        InstantiatedRoom instantiatedRoom)
    {
        ...
        if (neighborNodeXPosition >= instantiatedRoom.room.templateUpperBounds.x - instantiatedRoom.room.templateLowerBounds.x || 
            neighborNodeXPosition < 0 || neighborNodeYPosition >= instantiatedRoom.room.templateUpperBounds.y - instantiatedRoom.room.templateLowerBounds.y 
