...
namespace Pathfinding
{
    ...
    public class AIRigidbody2D : VersionedMonoBehaviour, IAstarAI
    {
        [HideInInspector] Rigidbody2D rb2D;

        Seeker seeker;

        [SerializeField] private float repathOnDisplacementThreshold = 0.5f;
        ...
        /// <summary>
        /// Starts searching for paths. cIf you override this function you should in most cases call base.Start () at the start of it.
        /// See: <see cref="Init"/>
        ...
        public float repathRate { get { return autoRepath.interval; } set { autoRepath.interval = value;} }
        public bool canSearch 
        { 
            get { return autoRepath.mode != AutoRepathPolicy.Mode.Never; } 
        ...
        public bool reachedDestination
        {
            get
            {
                ...
                // In theory a check against 0 should be done, but this will be a bit more resilient against targets that move slowly or maybe
                // jitter around due to floating point errors.
        ...
        public bool updatePosition = true; // I CHANGED HERE
        ...
        public Vector3 position
        {
            get
            {
                return updatePosition ? rb2D.position : simulatedPosition;
            }
        }
        ...
        {
            get { return updateRotation ? Quaternion.Euler(0f, 0f, rb2D.rotation) : simulatedRotation; }
            set
            {
                if (updateRotation)
                {
                    rb2D.rotation = value.z;
                }
                else
                {
        ...
        public void Teleport(Vector3 position, bool clearPath = true)
        {
            ...
            if (updatePosition) rb2D.position = position;
        ...
        public void SetPath(Path path)
        {
            ...
            else if (path.PipelineState == PathState.Returned)
            {
                ...
                else throw new System.ArgumentException("If you calculate the path using seeker.StartPath then this script will pick up the " +
                    "calculated path anyway as it listens for all paths the Seeker finishes calculating. You should not call SetPath in that case.");
            ...
            else
            {
                ...
                throw new System.ArgumentException("You must call the SetPath method with a path that either has been completely calculated or one whose " +
                    "path calculation has not been started at all. It looks like the path calculation for the path you tried to use has been started, but is not " +
                    "yet finished.");
            }
        }
        ...
        private void FixedUpdate()
        {
            ...
            {
                isBoss = enemy.enemyDetails.isEnemyBoss;
            }
            else
            {
                EnemyNetwork enemyNetwork = GetComponent<EnemyNetwork>();
                isBoss = enemyNetwork.isBoss;
            }

            if (enemy != null && isBoss) return; // Skip motion if enemy is a boss
            ...
            if (canMove)
            {
                ...
                // Detect significant displacement (likely from force)
                float displacement = Vector2.Distance(rb2D.position, lastFramePosition);
                if (displacement > repathOnDisplacementThreshold && Time.time - lastRepathTime > repathCooldown)
                {
                    SearchPath(); // or: RequestRepath();
                    lastRepathTime = Time.time;
                }

                lastFramePosition = rb2D.position;
            }
        }
        ...
        public void FinalizeMovement(Vector3 nextPosition, Quaternion nextRotation, Rigidbody2D rigidBody2D = null)
        {
        ...
        public void RequestRepath(float delay = 0f)
        {
            if (delay <= 0f) SearchPath();
            else StartCoroutine(RequestRepathDelayed(delay));
        }

        IEnumerator RequestRepathDelayed(float delay)
        {
