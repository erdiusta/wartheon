            ...
            foreach (Collider2D col in colliders)
            {
                ...
                if (col.CompareTag(Settings.enemyTag))
                {
                    ...
                    if (enemy.health != null)
                    {
                        ...
                        HealthAuthorityResolver.GetAuthority(enemy.gameObject).ApplyDamage((int)(player.whirlrendDamage * enemyDamageModifier), ctx);
                        ...
                        HealthAuthorityResolver.GetAuthority(player.gameObject).ApplyDamage((int)(player.whirlrendDamage * playerDamageModifier), ctx);
                    }
                ...
                else if (col.CompareTag(Settings.practiceDummy) || col.CompareTag(Settings.environment))
                {
                    ...
                    HealthAuthorityResolver.GetAuthority(health.gameObject).ApplyDamage(player.whirlrendDamage, ctx);
                    HealthAuthorityResolver.GetAuthority(player.gameObject).ApplyDamage((int)(player.whirlrendDamage * 0.15f), ctx);
                }
            }
        }
    }
