...
public class InputManager : SingletonMonobehaviour<InputManager>
{
    ...
    private void Update()
    {
        ...
        {
            Vector2 currentMousePosition = pointerPosition.action.ReadValue<Vector2>();
    ...
    {
        return Keyboard.current.anyKey.wasPressedThisFrame || Mouse.current.leftButton.wasPressedThisFrame || Gamepad.current?.buttonSouth.wasPressedThisFrame == true;
    }
    ...
    public bool AnyUIConfirmIntent()
    {
        ...
        if (gamepadAim.action.WasPressedThisFrame()) return false;
        if (interaction.action.WasPressedThisFrame()) return false;
    ...
    private bool AnyGamepadButtonPressed()
    {
        ...
        foreach (var control in Gamepad.current.allControls)
            if (control is ButtonControl btn && btn.wasPressedThisFrame) return true;

        return false;
    }

    public void ResetUIConfirm()
    {
        uiConfirmConsumed = false;
    }
