...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            foreach (InputAction otherAction in action.actionMap.actions)
            {
                ...
                for (int i = 0; i < otherAction.bindings.Count; i++)
                {
                    ...
                    {
                        otherAction.ApplyBindingOverride(i, oldOverridePath); // Swap bindings to prevent after reset
                    }
                }
            }
        }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            if (uiNavigation != null && uiNavigation.action != null) uiNavigation.action.Disable();
  
            //Fixes the "InvalidOperationException: Cannot rebind action x while it is enabled" error
            action.Disable();

            s_IsRebinding = true; // Set before starting rebind
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnCancel(
                    ...
                    {
                        action.Enable();
                        // Re-enable navigation here
                        uiNavigation?.action?.Enable();
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        // Re-enable navigation here too
                        if (uiNavigation != null && uiNavigation.action != null) uiNavigation.action.Enable();
        ...
        private void CheckOverriddenBindingsPerformedByPlayer(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            InputBinding newBinding = action.bindings[bindingIndex];
            ...
            foreach (InputBinding binding in action.actionMap.bindings)
            {
                ...
                {
                    string overriddenActionName = binding.action;
                    InputAction overriddenAction = action.actionMap.FindAction(overriddenActionName);

                    if (overriddenAction != null)
                    {
                        int indexToRemove = -1;

                        for (int i = 0; i < overriddenAction.bindings.Count; i++)
                        {
                            if (overriddenAction.bindings[i].id == binding.id) // Matching by ID is most reliable
                            {
                                indexToRemove = i;
                                break;
                            }
        ...
        private static bool s_IsRebinding = false;
        public static bool IsRebinding => s_IsRebinding;
        ...
        [SerializeField] string m_ActionLabelString;
