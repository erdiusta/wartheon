...
namespace Pathfinding {
	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI {
		...
		/// if it does not know. This is important only for the point graph which does not have a well defined up direction. The other built-in 
		/// graphs (e.g the grid graph)
		...
		public void FinalizeMovement (Vector3 nextPosition, Quaternion nextRotation, Rigidbody2D rb2D = null) {
            ...
            if (updatePosition)
            {
				...
				{
                    tr.position = nextPosition;       // Only fallback
                }
            }
